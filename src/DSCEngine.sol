...
contract DSCEngine is ReentrancyGuard {
    ...
    function depositCollateral(address tokenCollateralAddress, uint256 amountCollateral)
        ...
        nonReentrant
    {
        // Tracks how much of each collateral token each user has deposited.
        s_collateralDeposited[msg.sender][tokenCollateralAddress] += amountCollateral;
        emit CollateralDeposited(msg.sender, tokenCollateralAddress, amountCollateral);
        // the contract actually pulls the tokens from the user’s wallet into the DSCEngine contract.
        bool success = IERC20(tokenCollateralAddress).transferFrom(msg.sender, address(this), amountCollateral);
        if (!success) {
            revert DSCEngine__CollateralTransferFailed();
        }
        // Even though you update the mapping BEFORE the transfer, if transferFrom fails, the whole transaction is reverted, so mapping is never actually updated in final state.
    }
    ...
    function mintDsc(uint256 amountDscToMint) external moreThanZero(amountDscToMint) nonReentrant {
        ...
        // Check: If the minted too much, health factor eill be broken
    ...
    ////////////////////////////////////////
    // Private & Internal View Functios   //
    ////////////////////////////////////////
